...
export default function AlterClientForm() {
  ...
  useEffect(() => {
    const fetchClient = async () => {
      try {
        const clients = await findClients(); // Chama a função para buscar todos os clientes
        const clientToUpdate = clients.find(
          (client) => client.id.toString() === id
        ); // Busca o cliente com o ID selecionado

        console.log('Clientes encontrados:', clients); // Para ver todos os clientes retornados
        console.log('Cliente a ser atualizado:', clientToUpdate);

        if (clientToUpdate) {
          reset(clientToUpdate); // Reseta o formulário com os dados do cliente encontrado
        } else {
          console.log('Cliente não encontrado');
        }
      } catch (error) {
        console.error('Erro ao buscar cliente:', error); // Lida com erros durante a busca
      }
    };

    fetchClient(); // Chama a função de busca
  }, [id, reset]);
