...
export class ValidPassword implements IStrategy { //Atualizar está permitindo senhas inválidas
    process(entity: EntityDomain): object {
        ...
        if(entity instanceof User){
            ...
            if(user.getPassword() !== user.getConfirmPassword() || !this.validCharactersPassword(entity.getPassword())){
                return {
                    error: "Passwords isn't valid ! Verify if the password contains characters: A-Z, a-z, 0-9, special character, length > 8 or if passwords is equals"
                }
            }
    ...
    private validCharactersPassword(password: string): boolean{
       ...
       return regexLetters.test(password) && regexNumber.test(password) && regexCharacters.test(password)
    }
}
