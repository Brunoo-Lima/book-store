...
export class ClientDao implements IDao {
        ...
        return await prisma.client.update({
            data: {
                ...
                cli_profilePurchase: client.profilePurchase
                    ? { set: client.profilePurchase as string }
                    : undefined,
                cli_ranking: client.ranking
                    ? { set: client.ranking }
                    : undefined,
                ...
                cli_status: client.statusClient
                    ? { set: client.statusClient as string }
                    : undefined,
                cli_address: {
                    updateMany: client.addresses
                        ...
                        .map((address) => ({
                            ...
                            data: {
                                add_name: address.nameAddress
                                    ? { set: address.nameAddress }
                                    : undefined,
                                add_streetName: address.streetName
                                    ? { set: address.streetName }
                                    : undefined,
                                add_publicPlace: address.publicPlace
                                    ? { set: address.publicPlace }
                                    : undefined,
                                add_number: address.number
                                    ? { set: address.number }
                                    : undefined,
                                ...
                                add_state: address.state
                                    ...
                                    : undefined,
                                add_compostName: address.compostName
                                    ? { set: address.compostName }
                                    : undefined,
                                add_typeResidence: address.typeResidence
                                    ? { set: address.typeResidence as string }
                                    : undefined,
                    ...
                    createMany: {
                        data: client.addresses
                            ...
                            .map((address) => ({
                                ...
                                add_compostName: address.compostName,
                                add_typeResidence:
                                    address.typeResidence as string,
                                add_isBilling: address.change as boolean,
                                add_isDelivery: address.delivery as boolean,
                            })),
                    },
                ...
                cli_creditCards:
                    client.creditCard && client.creditCard.length !== 0
                        ? {
    ...
    public async findMany(client: Client): Promise<unknown> {
        ...
        if (client.gender && client.gender !== Gender.NULL)
            filter.cli_gender = {
                equals: client.gender,
            };
        if (client.statusClient && client.statusClient !== StatusClient.NULL)
            filter.cli_status = {
        ...
        if (client.phones && client.phones.length > 0) {
            filter.cli_phone = {
                some: {
                    pho_number: {
                        in: client.phones.map((phone) => phone.number),
                    },
